...
// if any check box is selected we post it, store that in session  
  ...
  if(isset($_POST['sort_page'])) {
	$sort_items_by = $_POST['sort_page'] ;  
	$_SESSION["sort_by"] = " order by $sort_items_by"  ;
	$_SESSION["sort"] = $sort_items_by ;
  }
...
// clear all sessions   
  if(isset($_POST["features"])){
	$mychecks1 = $_POST["features"] ;
		if ($mychecks1 == 0) {	
			unset($_SESSION["featuretype"]);
...
if(isset($_SESSION["featuretype"])){
 
	if (!in_array("smart", $_SESSION["featuretype"])){
			if(isset($_SESSION["smart"])){
			unset($_SESSION["smart"]);
		}
	}
	
	if (!in_array("3D", $_SESSION["featuretype"])){
...
// if checkbox is unckeched, delete its session for price range Weight
...
###############################################################################################
  ...
  if (empty($opts) AND empty($ftype) AND empty($pricetype)){
    $where .= ' AND TRUE';
  } 
...
##############################################################################################
  ...
  if(!empty($opts) AND empty($ftype) AND empty($pricetype)){
	foreach ($opts as $opt) {
		if ($opt == 'vision_fresh' OR $opt == 'sattvic' OR $opt == 'organic_tattva' OR $opt == 'down_to_earth' OR $opt == 'conscious_food' ) {
     	$tmp[] = '"'.$opt.'"';
		$_SESSION[$opt] = $opt;
		}
	}
...
##############################################################################################
######### if only feature Weight selected  #####################################################
##############################################################################################
  
  if(empty($opts) AND !empty($ftype) AND empty($pricetype)){
	if (in_array("smart", $ftype)){
		$typecounter = $typecounter + 1;
	}
	
	if (in_array("3D", $ftype)){
		$typecounter = $typecounter + 1;
	}
...
##############################################################################################
 ...
 if (empty($opts) AND empty($ftype) AND !empty($pricetype)) {
	...
	{
			if (in_array("cheap", $pricetype)){
				$where .= " AND Product_Price >= 50 AND Product_Price < 100";
			...
			if (in_array("expensive", $pricetype)){
				$where .= " AND Product_Price >= 200 AND Product_Price < 1000";
	...
	{	
		if ((in_array("cheap", $pricetype)) AND (in_array("medium-priced", $pricetype))){
			$where .= " AND ((Product_Price >= 50 AND Product_Price < 100) OR (Product_Price >= 100 AND Product_Price < 200 ))";
			if(!isset($_SESSION["cheap"])){
				$_SESSION["cheap"] = "cheap";
			}
			if(!isset($_SESSION["medium"])){
		...
		if ((in_array("cheap", $pricetype)) AND (in_array("expensive", $pricetype))){
			$where .= " AND ((Product_Price >= 100 AND Product_Price < 200) OR (Product_Price >= 200 AND Product_Price < 1000 ))"; 
			if(!isset($_SESSION["cheap"])){
				$_SESSION["cheap"] = "cheap";
			}
			if(isset($_SESSION["medium"])){
	...
	if($pricecounter == 3)
	{	
		$where .= " AND ((Product_Price >= 50 AND Product_Price < 100) OR (Product_Price >= 100 AND Product_Price < 200 ) OR (Product_Price >= 200 AND Product_Price < 1000))";
			if(!isset($_SESSION["expensive"])){
				$_SESSION["expensive"] = "expensive";
			}	
			if(!isset($_SESSION["cheap"])){
...
if (!empty($opts) AND empty($ftype) AND !empty($pricetype)){
	...
	{	
		...
		if ((in_array("cheap", $pricetype)) AND (in_array("expensive", $pricetype))){
			$where .= " AND ((Product_Price >= 50 AND Product_Price < 100) OR (Product_Price >= 200 AND Product_Price < 1000 ))"; 
			if(!isset($_SESSION["cheap"])){
				$_SESSION["cheap"] = "cheap";
			}
			if(isset($_SESSION["medium"])){
...
# If checkbox/filter selected, store that in session and build query based on filter/checkbox selected   
 elseif (!empty($opts) AND !empty($ftype) AND !empty($pricetype)) 
  {
	...
	{	
		...
		if ((in_array("cheap", $pricetype)) AND (in_array("expensive", $pricetype))){
			$where .= " AND ((Product_Price > 50 AND Product_Product_Price < 100) OR (Product_Price >= 200 AND Product_Price < 1000 ))"; 
			if(!isset($_SESSION["cheap"])){
				$_SESSION["cheap"] = "cheap";
			}
			if(isset($_SESSION["medium"])){
	...
	if($pricecounter == 3)
	{	
		$where .= " AND ((Product_Price >= 100 AND Product_Price < 200) OR (Product_Price >= 100 AND Product_Price < 200 ) OR (Product_Price >= 200 AND Product_Price < 1000))"; 
		if(!isset($_SESSION["cheap"])){
			$_SESSION["cheap"] = "cheap";
		}	
  ...
  if ($pn < 1) { // If it is less than 1
	...
	if(isset($_SESSION["sort_by"])){
		$query = $query . $_SESSION["sort_by"] . $limit;
	}
	else{
		$query = $query . $limit;
	}	
...
if ($productCount > 0) {
	...
	$types = array('price','Sort by');
	$type = isset($_SESSION['sort'])?$_SESSION['sort']:'Sort By';
	$type = isset($_SESSION['sort']) && in_array($_SESSION['sort'],$types)?$_SESSION['sort']:'Sort by';
	
	$dynamicList .= '
					<select id="mySelect" onchange="myFunction()">';
	 foreach($types as $option) {
     $dynamicList .= '<option value="'.$option.'"'.(strcmp($option,$type)==0?' selected="selected"':'').'>'.$option.'</option>';
				}    
     $dynamicList .=  '</select>';					
					/*
						  <option value="select">Sort by
						  
						  <option value="price_desc">Price High to low
						  
						<option value="price" if( isset(' . $_SESSION["sort"] . ') && ' . $_SESSION['sort'] . ' == "price"){selected="selected"}> price
